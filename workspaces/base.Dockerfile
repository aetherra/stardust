...
RUN apt-get update && apt-get install --no-install-recommends -y \
  xfonts-75dpi xvfb wmctrl x11vnc passwd dbus dbus-x11 libxrandr2 libxext-dev libxrender-dev libxtst-dev imagemagick x11-apps build-essential pulseaudio gstreamer1.0* fonts-noto-color-emoji \
