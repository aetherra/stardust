...
export default class WebAudio {
	url: string;
	connected: boolean;
	maximumAudioLag: number;
	syncLagInterval: number;
	updateBufferEvery: number;
	reduceBufferInterval: number;
	maximumSecondsOfBuffering: number;
	...
	reduceBuffer() {
		if (
			!(this.buffer && !this.buffer.updating && !!this.audio && !!this.audio.currentTime && this.audio.currentTime > 1)
		) {
			return;
		}
	...
	syncInterval() {
		...
		const targetTime = this.buffer.buffered.end(this.buffer.buffered.length - 1);

		if (targetTime > currentTime + this.maximumAudioLag) this.audio.fastSeek(targetTime);
	}

	//joins two data arrays - helper function
	concat(buffer1: ArrayBufferLike, buffer2: ArrayBufferLike) {
